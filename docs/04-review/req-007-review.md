...
| C | 테스트 결과 | Pass | UT-007-01~16 전수 Pass (16/16). 전체 회귀 테스트 86개 Pass. TST-007-01(팩토리 분기) Pass, TST-007-02(SSE 인터페이스 동일성) Pass. TST-007-03 / TST-007-04는 인증 환경·API 키 미충족으로 Skip — 핵심 동작은 mock 기반 단위 테스트로 검증됨 |
...
| TST-007-04 | E2E | 두 백엔드 전환 시 프론트엔드 동작 동일 확인 | Skip | 실제 API 키 및 HWP 파일 필요. 프론트엔드 무변경 원칙은 SSE 인터페이스 동일성(TST-007-02)으로 검증됨 |
...
- TST-007-03, TST-007-04: Skip (인증 환경·API 키 미충족 — 핵심 동작은 mock 기반 테스트로 검증됨)
- TST-SEC-08~12: Pass
